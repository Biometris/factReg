...
#'
#' @param dat A data-frame with data from multi-environment trials.
...
#' @param G The column in dat containing the factor genotype (either character or
#' numeric).
#' @param E The column in dat containing the factor environment (either character
#' or numeric).
#' @param indices The columns in dat containing the environmental indices (vector
...
#' @param indicesData An optional data-frame containing environmental indices
...
#' @param main.covariates Optional. The columns in dat containing main covariates,
#' i.e. with effects that are constant across genotypes. May overlap with
#' indices.
#' @param main.cov.pen Numeric vector corresponding to main.covariates,
...
#' @param weight Numeric vector of length nrow(dat), specifying the weight
...
#' @param partition data.frame with columns E and partition. The column E
...
#' @param nfolds Default NULL. If partition == NULL, this can be used to specify the
...
#' "no" : No scaling.
#' @param postLasso boolean; default FALSE. Indicates whether to refit
#' the model without penalty, using only the variables selected by
#' lasso. To do: Require that alpha = 1.
#' @param quadratic boolean; default FALSE. If TRUE, quadratic terms (i.e., squared indices) are
...
#' @param verbose boolean; default FALSE. If TRUE, the accuracies per
...
#' \item{main.par}{If the option main.covariates is used, this vector contains estimates
#'            of the main effects of the indices specified in main.covariates.}
#' \item{trainAccuracyEnv}{a data-frame with the accuracy (r) for each training
#'  environment, as well as the root mean squre error (RMSE), mean abosolute
#'  deviation (MAD) and rank (the latter is a proportion: how many of the best
#'  5 genotypes are in the top 10). To be removed or further developed. All
#'  these quantities are also evaluated for a model with only genotypic and
#'  environmental main effects (columns rMain, RMSEmain and rankMain).}
...
GnE <- function(dat,
  ...
  options(na.action='na.pass') # important when there are NA's in the test set
  ...
  if (!is.null(main.covariates)) {
    ncol.ma.old <- ncol(ma)
    ma <- cbind(ma, Matrix::Matrix(as.matrix(rbind(dTrain,dTest)[, main.covariates])))
  }
  ...
  if (quadratic) {
    ...
    # If there are main.covariates, these are not included here
  ...
  penaltyFactorA[(nEnvTrain + nEnvTest + 1):(nEnvTrain + nEnvTest + nGenoTrain - 1)] <- penG

  if (!is.null(main.covariates)) {
    if (quadratic) {
      temp.ind <- c((ncol.ma.old + 1):(ncol.ma.old + length(main.covariates)), (ncol(ma) - length(main.covariates) + 1):ncol(ma))
        #(ncol(ma) - 2 * length(main.covariates) + 1):ncol(ma)
    } else {
      temp.ind <- (ncol(ma) - length(main.covariates) + 1):ncol(ma)
    }
    penaltyFactorA[temp.ind] <- main.cov.pen
  } else {
    temp.ind <- integer()
  }

  # note: even if unpenalized, the estimated main effects change, depending on lambda!

  # run glmnet, either (if provided) for a single value of lambda (using glmnet),
  # or using cv.glmnet

  thresh <- 1e-07

  if (lambdaProvided && length(lambda) == 1) {
    if (lambda == 0) {thresh <- 1e-11}
    glmnetOutA <- glmnet::glmnet(x = ma[1:nrow(dTrain),], y = dTrain$Y,
                                 lambda = lambda,
                                weights = dTrain$W,
    ...
    if (!is.null(main.covariates)) {
      main.par <- glmnetOutA$beta[temp.ind]
      names(main.par) <- colnames(ma)[temp.ind]
    } else {
      main.par <- NULL
    }
    ...
    if (postLasso) {
      lasso.model <- glmnet::glmnet(x = ma , y = dTrain$Y, alpha = 1,
                                    weights = dTrain$W,
                                    family = "gaussian",
    ...
    if (!is.null(main.covariates)) {
      main.par <- glmnetOutA$glmnet.fit$beta[temp.ind, lambdaIndex]
    } else {
      main.par <- NULL
    }
  ...
  temp.ind.2 <- setdiff((nEnvTrain + nEnvTest + nGenoTrain):ncol(ma), temp.ind)

  ## assign the genotypic main effects
  parGeno <- data.frame(main = c(0, cfe[(nEnvTrain + nEnvTest + 1):(nGenoTrain + nEnvTrain + nEnvTest - 1),
                                        lambdaIndex]), row.names = levels(dTrain$G))

  ## assign the genotype specific sensitivities (subsequent columns)
  #cfeIndRows <- rownames(cfe[(nGenoTrain + nEnvTrain + nEnvTest):(nrow(cfe) - length(main.covariates)), , drop = FALSE])
  cfeIndRows <- rownames(cfe[temp.ind.2, , drop = FALSE])
  ...
  #                     val = cfe[(nGenoTrain + nEnvTrain + nEnvTest):(nrow(cfe) - length(main.covariates)), lambdaIndex],
  #                     stringsAsFactors = FALSE)
  cfeDf <- data.frame(geno = cfeGenotypes, index = cfeIndices,
                      val = cfe[temp.ind.2, lambdaIndex],
                      stringsAsFactors = FALSE)
  ...
  if (!is.null(testEnv)) {
    predTest <- as.numeric(predict(object = glmnetOutA,
  ...
  indFrameTrain2 <- merge(indFrameTrain, envMain2, by.x = "E", by.y = "row.names")
  ...
  if (is.null(partition)) {
    glmnetOut <- glmnet::cv.glmnet(x = as.matrix(indFrameTrain[, indices]),
                                   y = indFrameTrain$envMainFitted, alpha = alpha,
                                   nfolds = nfolds)
    glmnetOut2 <- glmnet::cv.glmnet(x = as.matrix(indFrameTrain2[, indices]),
                                   y = indFrameTrain2$envMainFitted, alpha = alpha,
                                   nfolds = nfolds)
  ...
  s3 <- split(as.numeric(predict(model.main, newx = mm)), dTrain$E)
  ...
  trainAccuracyEnv <-
    data.frame(Env = levels(dTrain$E),
               ...
               RMSE = mapply(FUN = function(x, y) {sqrt(mean((x - y) ^ 2, na.rm = TRUE))},
                             s1, s2),
               RMSEmain = mapply(FUN = function(x, y) {sqrt(mean((x - y) ^ 2, na.rm = TRUE))},
                                 s1, s3),
               MAD = mapply(FUN = function(x, y) {mean(abs(x - y), na.rm = TRUE)},
                            s1, s2),
               rank = mapply(FUN = exRank,
                             s1, s2),
               rankMain = mapply(FUN = exRank,
                                 s1, s3),
               row.names = NULL)
  ...
  if (!is.null(testEnv)) {
    ...
    s3t <- split(mainOnly[as.character(dTest$G)] + as.numeric(parEnvTest2[as.character(dTest$E), ]), dTest$E)
  ...
  out <- list(predTrain = predTrain, predTest = predTest,
              resTrain = resTrain, resTest = resTest,
              mu = mu,
              ...
              Y = Y, G = G, E = E,
              indices = indices,
              postLasso = postLasso, quadratic = quadratic)
